...
import com.freegang.fplugin.utils.reflect.findField
import com.freegang.fplugin.utils.reflect.findFieldAndGet
import com.freegang.fplugin.utils.reflect.findMethodAndInvoke
...
object PluginActivityBridge {
    ...
    fun hookAMS(packageName: String, className: String) {
        try {
            // 获取到 IActivityTaskManagerSingleton 字段
            val clazz = Class.forName("android.app.ActivityTaskManager")
            val singletonField = clazz.findField("IActivityTaskManagerSingleton")
            ...
            if (mInstance == null) {
                mInstance = singletonClazz.findMethodAndInvoke<Any>(singleton!!, "get")
            }
        ...
        try {
            ...
            mCallbackField.set(mH, object : Handler.Callback {
                override fun handleMessage(msg: Message): Boolean {
                    when (msg.what) {
                        159 -> { // Android 13 -> ActivityThread.H.EXECUTE_TRANSACTION = 159
    ...
    class InstrumentationProxy(private val instrumentation: Instrumentation) : Instrumentation() {
        ...
        fun execStartActivity(
            ...
            return execStartActivityMethod.invoke(instrumentation, who, contextThread, token, target, intent, requestCode, options) as ActivityResult?
        }
